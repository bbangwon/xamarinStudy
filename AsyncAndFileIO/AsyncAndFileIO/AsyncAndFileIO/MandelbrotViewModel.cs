...
namespace AsyncAndFileIO
{
    class MandelbrotViewModel : ViewModelBase
    {
        ...
        public MandelbrotViewModel(double baseWidth, double baseHeight)
        {
            ...
            Progress<double> progressReporter = new Progress<double>((double progress) => { Progress = progress; });
            CancellationTokenSource cancelTokenSource = null;

            CalculateCommand = new Command(execute: async () =>
            {
                IsBusy = true;
                ...
                try
                {
                    BitmapInfo = await model.CalculateAsync(TargetCenter,
                        baseWidth / TargetMagnification, baseHeight / TargetMagnification,
